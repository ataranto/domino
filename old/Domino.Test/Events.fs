...
module zero =
    ...
    let ``the Players map is empty`` () =
        zero |> (fun state -> state.Players) |> should be FsUnit.CustomMatchers.Empty

    [<Fact>]
    let ``the Active player is None`` () =
        zero |> (fun state -> state.Active) |> should equal None

    [<Fact>]
    let ``the zero state Winner optional value is None`` () =
        zero |> (fun state -> state.Winner) |> should equal None

    [<Fact>]
    let ``the zero state Board value is Empty`` () =
        zero |> (fun state -> state.Board) |> should equal Empty

    [<Fact>]
    let ``the zero state Bonyard value is an empty list`` () =
        zero |> (fun state -> state.Boneyard) |> should be FsUnit.CustomMatchers.Empty
...
module integration =
    let players =
        ...
        |> Seq.map (fun id ->
            { Id = id
              Name = sprintf "Player #%d" id })

    let rec run events =
        let state = events |> List.fold apply zero

        match state.Winner, state.Active with
        | Some winner, _ -> winner, events
        | None, Some player ->
            let tiles = state.Players |> Map.find player |> (fun ps -> ps.Tiles)

            let score =
                function
                | Lead tile -> Board.lead tile |> Board.score
                | Attach(tile, target) -> state.Board |> Board.attach tile target |> Board.score

            let action = state.Board |> Board.actions tiles |> List.maxBy score
    ...
    let ``a 2 player game can be completed and replayed`` () =
        let winner, events = players |> Seq.take 2 |> Seq.toList |> start |> run
    ...
    let ``a 4 player game can be completed and replayed`` () =
        let winner, events = players |> Seq.take 4 |> Seq.toList |> start |> run
