...
module lead =
    ...
    let ``when the tile is a double, it becomes the spinner`` () =
        ...
        tile
        |> lead
        |> should equal (Node(tile, [ 0, Empty; 0, Empty; 0, Empty; 0, Empty ]))

    [<Fact>]
    let ``when the tile is not a double, it does not becomes the spinner`` () =
        let tile = Tile(0, 1)

        tile |> lead |> should equal (Node(tile, [ 0, Empty; 1, Empty ]))
...
module attach =
    ...
    let ``when the target does not exist in the tree, the tree is not modified`` () =
        ...
        |> should equal (Node(tile, [ 0, Empty; 1, Empty ]))
    ...
    let ``when the tile can not be attached to the target, the tree is not modified`` () =
        ...
        |> lead
        |> attach tile target
        |> should equal (Node(target, [ 0, Empty; 1, Empty ]))
    ...
    let ``when the first double is attached, it becomes the spinner`` () =
        ...
        |> lead
        |> attach tile target
        |> should equal (Node(target, [ 0, Node(tile, [ 0, Empty; 0, Empty; 0, Empty ]); 1, Empty ]))
    ...
    let ``each value of a node can only have one child attached`` () =
        ...
        |> should equal (Node(target, [ 0, Node(tile, [ 2, Empty ]); 1, Empty ]))
    ...
    let ``no more than 2 tiles can be attached to a node`` () =
        ...
        |> should equal (Node(target, [ 1, Node(Tile(1, 2), [ 2, Empty ]); 0, Node(Tile(0, 2), [ 2, Empty ]) ]))
...
module actions =
    ...
    let ``when the board is empty, any tile can be played to lead`` () =
        ...
        Empty |> actions tiles |> should equal [ Lead tile ]
    ...
    let ``when a tile has an empty edge, a matching tile can be attached`` () =
        ...
        target |> lead |> actions tiles |> should equal [ Attach(tile, target) ]
    ...
    let ``when none of the tiles may be played, an empty list is returned`` () =
        ...
        target |> lead |> actions tiles |> should be FsUnit.CustomMatchers.Empty

module score =
    [<Fact>]
    let ``when no tiles have been played, the score is 0`` () = Empty |> score |> should equal 0
    ...
    let ``when the spinner is played first, it is counted correctly`` () =
        ...
        [ attach (Tile(3, 6)) tile; attach (Tile(2, 6)) tile; attach (Tile(5, 6)) tile ]
